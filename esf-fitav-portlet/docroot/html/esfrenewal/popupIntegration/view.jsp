...
PortletURL viewassURL = renderResponse.createRenderURL();
	...
	viewassURL.setParameter(
		"mvcPath", templatePath+"popupIntegration/view.jsp");
	viewassURL.setParameter("code", code);
	viewassURL.setParameter("name", name);

%>
<liferay-portlet:renderURL varImpl="searchURL">
	<portlet:param name="mvcPath" value='<%=templatePath+"popupIntegration/view.jsp"%>' />
...
<liferay-ui:search-container delta="5" emptyResultsMessage="no-results" iteratorURL="<%=viewassURL%>">
	<liferay-ui:search-container-results
		results="<%=ESFOrganizationLocalServiceUtil.findAllOrganizationsIncrements(name,code,searchContainer.getStart(),searchContainer.getEnd())%>"
		total="<%=ESFOrganizationLocalServiceUtil.findAllOrganizationsIncrements(name,code)%>" />
	...
	<liferay-ui:search-container-row
		...
		<liferay-ui:search-container-column-text name="name"/>
	 	...
	 	<liferay-ui:search-container-column-jsp
			path='<%=templatePath + "popupIntegration/choose_association_Integration.jsp"%>'
			align="right" /> 
	</liferay-ui:search-container-row>
