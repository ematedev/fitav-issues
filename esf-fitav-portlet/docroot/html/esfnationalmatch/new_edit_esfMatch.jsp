			...
			<%
				...
				for (int i=0;i< countries.size();i++) {
					...
					if(Validator.isNotNull(esfMatch) && esfMatch.getCountryId()==country.getCountryId())
						 selected = true;
					String nation =  country.getName().substring(0,1).toUpperCase() + country.getName().substring(1,country.getName().length()).toLowerCase();
			%>
					<aui:option label="<%=nation%>"	 value="<%=country.getCountryId()%>" 	selected="<%=selected%>" />
			<%
				}
			%>
		</aui:select>
