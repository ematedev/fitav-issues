<liferay-ui:search-container emptyResultsMessage="no-fields-for-this-organization">
				...
				<liferay-ui:search-container-results
					results="<%= ESFFieldLocalServiceUtil.findByESFOrganization(currentOrganizationId,ESFKeys.ESFStateType.ENABLE, searchContainer.getStart(), searchContainer.getEnd()) %>"
					total="<%= ESFFieldLocalServiceUtil.findByESFOrganization(currentOrganizationId,ESFKeys.ESFStateType.ENABLE).size()%>" />
				...
				<liferay-ui:search-container-row
					...
					ESFOrganization realOwnerOrg = ESFOrganizationLocalServiceUtil.getRealOwner(
						ESFKeys.ESFStateType.GIVEWAY, esfField.getEsfFieldId());
					%>
	
					<% if(realOwnerOrg != null){ 
						cededField = esfField.getName()+"         (ottenuto da: "+realOwnerOrg.getName()+")";%>
						
						<liferay-ui:search-container-column-text name="name" value="<%= cededField %>"/>
					<%}else{ %>
					...
					<c:if test="<%= ESFFieldPermission.contains(permissionChecker, esfField.getEsfFieldId(), ActionKeys.ESFFIELD_VIEW_ACTION_ENABLE) %>">
