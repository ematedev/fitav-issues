...
<liferay-ui:search-container emptyResultsMessage="no-result"
	iteratorURL="<%=viewassURL%>" curParam="sc1">
	<liferay-ui:search-container-results>
		<%
			List<ESFUserESFUserRole> allESFUserESFUserRole = ESFUserESFUserRoleLocalServiceUtil
							.getAllESFUserESFUserRoleByESFUserId(esfUserId, searchContainer.getStart(),
									searchContainer.getEnd());

					int totalESFUserESFUserRole = ESFUserESFUserRoleLocalServiceUtil
							.getAllESFUserESFUserRoleCountByESFUserId(esfUserId);
	...
	<liferay-ui:search-container-row
		className="it.ethica.esf.model.ESFUserESFUserRole"
		modelVar="esfUserESFUserRole">

		<%
			ESFUserRole esfUserRole = ESFUserRoleLocalServiceUtil
							.getESFUserRole(esfUserESFUserRole.getEsfUserRoleId());
					Organization org = null;
					String orgName = "";
					if (Validator.isNotNull(esfUserESFUserRole.getEsfOrganizationId())
							&& Validator.isNotNull(OrganizationLocalServiceUtil
									.fetchOrganization(esfUserESFUserRole.getEsfOrganizationId()))
							&& Validator.isNotNull(OrganizationLocalServiceUtil
									.fetchOrganization(esfUserESFUserRole.getEsfOrganizationId()).getName())) {

						orgName = OrganizationLocalServiceUtil
								.fetchOrganization(esfUserESFUserRole.getEsfOrganizationId()).getName();
		...
		<c:if
			test="<%=ESFShooterPermission.contains(permissionChecker, esfUserId,
								ActionKeys.ESFSHOOTER_ROLE_DELETE)%>">

			<liferay-ui:search-container-column-jsp
				path='<%="/html/esfshooter/navigation/roleRemove_actions.jsp"%>'
				align="right" />
		</c:if>
...
<c:if
	test="<%=ESFShooterPermission.contains(permissionChecker, esfUserId,
						ActionKeys.ESFSHOOTER_ROLE_VIEW_ACTION)%>">
	...
	<liferay-ui:search-container orderByType="<%=orderByType%>"
		emptyResultsMessage="no-results" curParam="sc2">
		<liferay-ui:search-container-results>

			<%
				List<ESFUserRole> allEsfUserRolePerPage = ESFUserRoleLocalServiceUtil
									.getAllEsfUserRoleByStateNoAssign(esfUserId, searchContainer.getStart(),
											searchContainer.getEnd());

							int totalEsfUserRole = ESFUserRoleLocalServiceUtil
									.countAllEsfUserRoleByStateNoAssign(esfUserId);
		...
		<liferay-ui:search-container-row
			...
			<liferay-ui:search-container-column-jsp
				path='<%="/html/esfshooter/navigation/esfUserRole_action.jsp"%>'
				align="right" />
