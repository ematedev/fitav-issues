...
viewassURL.setParameter(
	"mvcPath", "/html/match/edit_esfDoctor.jsp");
...
<c:choose>
	<c:when
		...
		<h3>
			<liferay-ui:message key="doctor-matches-of-tournament">
				<%=tournamentTitle%></liferay-ui:message>
		</h3>

		<b>
			<liferay-ui:message key="doctor-partecipant-assegn" />
		</b>
		...
		esfUsers = ESFUserLocalServiceUtil.findNationalPartecipantDoctor(
								esfTournamentId,
								esfMatchId, true);
...
<%-- 					value="<%=String.valueOf(isNational)%>" /> --%>
				<portlet:param name="esfPartecipantTypeId"
					value="<%=String.valueOf(ESFKeys.ESFNationalDelegationType.DOCTOR)%>" />
...
<%-- 					value="<%=String.valueOf(isNational)%>" /> --%>
		...
		<liferay-ui:search-container emptyResultsMessage="no-results" iteratorURL="<%=viewassURL %>">
			...
			esfPartecipants = ESFUserLocalServiceUtil.findNationalPartecipantDoctor(
				esfTournamentId, esfMatchId, false,
				searchContainer.getStart(),
				searchContainer.getEnd());
			
			esfPartecipantsCount =  ESFUserLocalServiceUtil.findNationalPartecipantDoctor(
