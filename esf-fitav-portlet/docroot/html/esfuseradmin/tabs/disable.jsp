...
<liferay-ui:search-container emptyResultsMessage="no-result" orderByType="<%=orderByType%>">
	<liferay-ui:search-container-results>
		<%
			String firstname="";
			String lastname="";
			int typestate=ESFKeys.ESFStateType.DISABLE;
				
			List<ESFUser> esfusers =
				ESFUserLocalServiceUtil.getAllUserByLikeF_L_S(firstname, lastname, "", typestate);
			
			List<ESFUser> esfusersSE = 
				ESFUserLocalServiceUtil.getAllUserByLikeF_L_S(firstname, lastname, "",typestate, searchContainer.getStart(), searchContainer.getEnd());
			...
			if (Validator.isNotNull(orderByCol)) {
				...
				Collections.sort(ordableEsfusersSE, beanComparator);

				if (sortingOrder.equalsIgnoreCase("desc")) {

						Collections.reverse(ordableEsfusersSE);
				}
			}
