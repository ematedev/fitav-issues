	...
	<c:choose>
		...
		<c:otherwise>
			<div class="taglib-form-navigator row-fluid" id="<portlet:namespace />tabs">
				...
				<ul class="form-navigator nav nav-list span4 well">
					...
					<c:if test="<%= showButtons %>">
						<aui:button-row>
							<aui:button primary="<%= true %>" type="submit"  name="submit" />
			...
			<aui:script use="aui-event-input,aui-tabview,aui-url,history,io-form">
				...
				var submitButton = A.one('.fitav-navigator #<portlet:namespace />submit');
				var editOrganizationDisplay = A.one('#<portlet:namespace />editOrganizationDisplay');
				
				if(editOrganizationDisplay){
