...
public class ESFOrganizationDisplay extends MVCPortlet {
	...
	public void editESFOrganization(
		...
		_log.info("idFatherAssociation="+idFatherAssociation);
		...
		if (esfOrganizationId > 0) {
			_log.info("ESFOrganizationDisplay Updating Organization " +
				esfOrganizationId);

			if (checkType(esfOrganizationId) == ESFKeys.ESFOrganizationTypeId.ASSOCIATION &&
				esfStateId != ESFKeys.ESFStateType.DISABLE &&
				!checkBdo(esfOrganizationId)) {
				SessionErrors.add(request, "organiz-bdo-message");
			}
				...
				if(!oldOrg.getName().equals(name) || !oldOrg.getCode().equals(code) ||
						!(esfEntityStateOld.getEsfEntityStateId() == esfStateId) || !(oldOrg.getVariations() == variationId)){
			...
			_log.info("ESFOrganizationDisplay Adding Organization");
			...
			ESFOrganizationLocalServiceUtil.addESFOrganization(
				...
				esfEntityState, variationId, regionCode, serviceContext);
	...
	// private static List<Phone> getPhones(ActionRequest actionRequest) {
	//
	...
	//
	// Phone phone = PhoneLocalServiceUtil.createPhone(phoneNumberId);
	//
