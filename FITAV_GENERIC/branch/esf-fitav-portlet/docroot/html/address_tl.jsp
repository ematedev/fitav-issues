	...
	<aui:fieldset>
		...
		<div class="row-fields">
			...
			<aui:col width="<%= 50 %>">
				...
				<aui:input name='<%= "street1"+iesima%>' label="street" type="text"
					value='<%=esfAddress == null ? "" : esfAddress.getStreet1()%>' <%= readOnly ? "readonly" : "" %> >
				...
				<aui:input name='<%= "zip"+iesima%>' label="zip" type="text"
					value='<%=esfAddress == null ? "" : esfAddress.getZip()%>'  <%= readOnly ? "readonly" : "" %> >
		...
		<c:choose>
			...
			<c:otherwise>
				<aui:select id='<%= "regions"+iesima%>' name='<%="esfRegionId"+iesima%>' label="regions" required="<%= requiredField %>"></aui:select>
				<aui:select id='<%= "provinces"+iesima%>' name='<%= "esfProvinceId"+iesima%>' label="provinces" required="<%= requiredField %>"></aui:select>
				<aui:select id='<%= "cities"+iesima%>' name='<%= "esfCityId"+iesima%>' label="cities" required="<%= requiredField %>"></aui:select>
			</c:otherwise>
		</c:choose>
