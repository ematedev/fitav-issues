...
List<ESFRenewal> renevals =  ESFRenewalLocalServiceUtil.findByOrg_Year(currentOrganizationId, thisYear);
...
String koPaymentYear = "outstanding_payments";
String okPaymentYear = "no_outstanding_payments";
...
stateChanged = ESFStateLocalServiceUtil.fetchESFState(esfEntityState.getEsfStateId()).getName()+"**";
}
...
int minShooter = Integer.parseInt( ESFConfigurationLocalServiceUtil.
					getESFConfigurationByESFKey("min-number-shooter-for-activate-association").getValue());

List<ESFUserESFUserRole> bdoUserRole = ESFUserESFUserRoleLocalServiceUtil.findESFUserESFRolesByEsfOrganizzationId(currentOrganizationId);

Role president = RoleLocalServiceUtil.getRole(themeDisplay.getCompanyId(), "Presidente Associazione");
Role secretary = RoleLocalServiceUtil.getRole(themeDisplay.getCompanyId(), "Segretario Associazione");					
String presidentValue =  ESFConfigurationLocalServiceUtil.getESFConfigurationByESFKey("presdent-is-required-for-activate-association").getValue();
String secretarytValue = ESFConfigurationLocalServiceUtil.getESFConfigurationByESFKey("secretary-is-required-for-activate-association").getValue();
		
		
		
for(ESFUserESFUserRole bdo : bdoUserRole){
	if( Boolean.valueOf( ESFConfigurationLocalServiceUtil.getESFConfigurationByESFKey("presdent-is-required-for-activate-association").getValue())
			&& (bdo.getEsfUserRoleId()==president.getClassPK())){
		missing_requirements_president = false;
	}
	if( Boolean.valueOf( ESFConfigurationLocalServiceUtil.getESFConfigurationByESFKey("secretary-is-required-for-activate-association").getValue())
			&& (bdo.getEsfUserRoleId()==secretary.getClassPK())){
		missing_requirements_secretary = false;
}

}
List<ESFUser>orgShooter =ESFUserLocalServiceUtil.getAllUserByLikeF_C_S(	"", "", "", ESFKeys.ESFStateType.ENABLE,
		currentOrganizationId);
							
if(orgShooter.size() < minShooter){
...
<c:when test="<%=currentESFOrganization != null%>">
	...
	List<ESFHistoricalAssociation> historyTot = ESFHistoricalAssociationLocalServiceUtil.findbyOrg_Date(currentOrganizationId);
	...
	if(historyTot.size()>0){
		...
		for (ESFHistoricalAssociation h : historyTot){
			...
			if(h.getVariationId()>0 && varId>0){
				...
				variationChanged =  ListTypeServiceUtil.getListType(varId).getName();
			}
			variation = variationChanged;
			
			String[] lastCangeName = {data, lastName};
			String[] lastCangeCode = {data, lastCode};
			if(!lastName.equals(currentESFOrganization.getName()) || !lastCode.equals(currentESFOrganization.getCode())){
			%>
			
			<%	
				if(!lastName.equals(currentESFOrganization.getName())){
				%>
				<h4><b><liferay-ui:message key="in-date-x-name-x"  arguments="<%= lastCangeName%>"/></b>
				<div class="separator"></div>
				<%	
				}
				if(!lastCode.equals(currentESFOrganization.getCode())){
				%>
				<h4><b><liferay-ui:message key="in-date-x-code-x"  arguments="<%= lastCangeCode%>"/></b>
				<div class="separator"></div>
				<%	
				}
			break;
	...
	if(missing_requirements_president || missing_requirements_secretary || missing_requirements_shooter ){
		%>	
	
	<h4><b><liferay-ui:message key="error_BDO" /> </b>
	<%
	if(missing_requirements_president){
	
	%>	
	<h4><b><liferay-ui:message key="<%=noPresident %>" /> </b>
	<%	
	}
	if(missing_requirements_secretary){
	%>
	<h4><b><liferay-ui:message key="<%=noSecretary %>" /></b>
	<%
	}
	if(missing_requirements_shooter){
	%>
	<h4><b><liferay-ui:message key="<%=noShooter %>" /></b>
	<%
	}
	%>
	<div class="separator"></div>
	...
	if(paymentOk){
	%>
	<h4><b><liferay-ui:message key="<%=okPaymentYear %>" /></b>
	<div class="separator"></div>
	
	<% }else{
		
	%>
	<h4><b><liferay-ui:message key="<%=koPaymentYear %>" /></b>
	...
	<h3><ins><liferay-ui:message key="field-address" /></ins></h3>
	...
	<div class = "row detail">
		<div class="span6" >
			...
			%>
			<p><liferay-ui:message key="is-multi-sport" /></p>
			<%
				} else {
			%>
			<p><liferay-ui:message key="is-mono-sport" /></p>
			<%
				}
			%>
		
		
			<h4><liferay-ui:message key="category-state" /></h4>
			<%
			/*List<ListType> categories = ListTypeServiceUtil
							.getListTypes(ESFOrganization.class.getName()
									+ ESFListType.CATEGORY);
			String categorySelect = "";
			String categoryId = currentESFOrganization
					.getCategory();
			for (ListType category : categories) {
				String currentId = String.valueOf(category
						.getListTypeId());
				if (categoryId.equals(currentId)) {
					categorySelect = category.getName();
					if (categorySelect.contains("-")
							&& categorySelect.split("-").length > 1) {
						categorySelect = categorySelect.split("-")[1];
					}
				}
			...
			%>
			<p><liferay-ui:message key="<%=categoryAssosiation%>" /></p>
		
			<h4><liferay-ui:message key="first-affiliation-date" /></h4>
			
			<p>
				<%=currentESFOrganization
							.getFirstAffiliationDate() == null ? ""
							: ManageDate
									.DateToString(currentESFOrganization
