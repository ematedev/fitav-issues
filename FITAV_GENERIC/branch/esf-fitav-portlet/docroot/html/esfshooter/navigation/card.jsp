...
long cardIdToString=0;
String namespace = renderResponse.getNamespace();
String associations;

%>
<portlet:renderURL var="chooseAssociationURL" windowState="<%=LiferayWindowState.POP_UP.toString()%>">
	<portlet:param name="mvcPath" value='<%="/html/esfshooter/popup/view.jsp"%>'></portlet:param>
</portlet:renderURL>
...
<div class="btn-field-align">
	<aui:input type="text" label ="actualCard" id = "actualCard" name="actualCard"  value='<%=cardCode%>' readonly="true" inlineField="true"/>
	...
	<aui:button-row>
		...
		<aui:button id="remove" value="remove-card" onClick='<%= removeCard.toString() %>' />
	</aui:button-row>
</div>
...
<c:choose>
	<c:when test="<%=esfUser != null%>">
		<liferay-ui:search-container emptyResultsMessage="no-card">
			...
			<liferay-ui:search-container-row className="it.ethica.esf.model.ESFCard" modelVar="card">
			
			<liferay-ui:search-container-column-text  property="code" value="<%= card.getCode()%>" />
				<%
					...
					%>
				<liferay-ui:search-container-column-text name="orgName" value="<%=orgName%>" />
				...
				<liferay-ui:search-container-column-text name="releaseYear" value="<%= creationYear %>" />
