...
<aui:button-row cssClass="esfcard-admin-buttons">
		...
		<c:if test="<%= ESFCardModelPermission.contains(permissionChecker, scopeGroupId, ActionKeys.ESFCARD_SELECT) %>">	
			...
			<aui:button onClick="<%= selectESFCardURL.toString() %>" value="assign-cards-2017" />
		</c:if>
...
<liferay-ui:search-container emptyResultsMessage="no-results">
		...
		if(Validator.isNull(esfOrganizationType) || esfOrganizationType < ESFKeys.ESFOrganizationTypeId.PROVINCE){
			...
			organizations = ESFOrganizationLocalServiceUtil.findAllChildOrganizations(esfOrganizationId,
					searchContainer.getStart(), searchContainer.getEnd());
			
			organizationsTot = ESFOrganizationLocalServiceUtil.findAllChildOrganizations(esfOrganizationId);
		
		}
		
		pageContext.setAttribute("results", organizations);
		pageContext.setAttribute("total", organizationsTot.size());
