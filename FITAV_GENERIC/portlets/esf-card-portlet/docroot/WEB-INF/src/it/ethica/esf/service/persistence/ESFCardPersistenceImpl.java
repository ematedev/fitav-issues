...
public class ESFCardPersistenceImpl extends BasePersistenceImpl<ESFCard>
	...
	public static final FinderPath FINDER_PATH_WITH_PAGINATION_FIND_BY_FINDBYSD_ED =
		new FinderPath(ESFCardModelImpl.ENTITY_CACHE_ENABLED,
			ESFCardModelImpl.FINDER_CACHE_ENABLED, ESFCardImpl.class,
			FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByfindBySD_ED",
			new String[] {
	...
	public static final FinderPath FINDER_PATH_WITHOUT_PAGINATION_FIND_BY_FINDBYSD_ED =
		new FinderPath(ESFCardModelImpl.ENTITY_CACHE_ENABLED,
			ESFCardModelImpl.FINDER_CACHE_ENABLED, ESFCardImpl.class,
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findByfindBySD_ED",
			new String[] { Date.class.getName(), Date.class.getName() },
			ESFCardModelImpl.STARTDATE_COLUMN_BITMASK |
			ESFCardModelImpl.ENDDATE_COLUMN_BITMASK);
	public static final FinderPath FINDER_PATH_COUNT_BY_FINDBYSD_ED = new FinderPath(ESFCardModelImpl.ENTITY_CACHE_ENABLED,
			ESFCardModelImpl.FINDER_CACHE_ENABLED, Long.class,
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByfindBySD_ED",
			new String[] { Date.class.getName(), Date.class.getName() });
	 ...
	 */
	@Override
	public List<ESFCard> findByfindBySD_ED(Date startDate, Date endDate)
		throws SystemException {
		return findByfindBySD_ED(startDate, endDate, QueryUtil.ALL_POS,
			QueryUtil.ALL_POS, null);
	}
	...
	public List<ESFCard> findByfindBySD_ED(Date startDate, Date endDate,
		...
		return findByfindBySD_ED(startDate, endDate, start, end, null);
	}
	...
	public List<ESFCard> findByfindBySD_ED(Date startDate, Date endDate,
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
				(orderByComparator == null)) {
			pagination = false;
			finderPath = FINDER_PATH_WITHOUT_PAGINATION_FIND_BY_FINDBYSD_ED;
			finderArgs = new Object[] { startDate, endDate };
		}
		else {
			finderPath = FINDER_PATH_WITH_PAGINATION_FIND_BY_FINDBYSD_ED;
			finderArgs = new Object[] {
					startDate, endDate,
		...
		if ((list != null) && !list.isEmpty()) {
			for (ESFCard esfCard : list) {
				if (!Validator.equals(startDate, esfCard.getStartDate()) ||
						!Validator.equals(endDate, esfCard.getEndDate())) {
					list = null;
		...
		if (list == null) {
			...
			if (startDate == null) {
				query.append(_FINDER_COLUMN_FINDBYSD_ED_STARTDATE_1);
			}
			else {
				bindStartDate = true;

				query.append(_FINDER_COLUMN_FINDBYSD_ED_STARTDATE_2);
			}

			boolean bindEndDate = false;

			if (endDate == null) {
				query.append(_FINDER_COLUMN_FINDBYSD_ED_ENDDATE_1);
			}
			else {
				bindEndDate = true;

				query.append(_FINDER_COLUMN_FINDBYSD_ED_ENDDATE_2);
			}
	 ...
	 */
	@Override
	public ESFCard findByfindBySD_ED_First(Date startDate, Date endDate,
		OrderByComparator orderByComparator)
		throws NoSuchCardException, SystemException {
		ESFCard esfCard = fetchByfindBySD_ED_First(startDate, endDate,
				orderByComparator);
	 ...
	 */
	@Override
	public ESFCard fetchByfindBySD_ED_First(Date startDate, Date endDate,
		OrderByComparator orderByComparator) throws SystemException {
		List<ESFCard> list = findByfindBySD_ED(startDate, endDate, 0, 1,
				orderByComparator);
	 ...
	 */
	@Override
	public ESFCard findByfindBySD_ED_Last(Date startDate, Date endDate,
		OrderByComparator orderByComparator)
		throws NoSuchCardException, SystemException {
		ESFCard esfCard = fetchByfindBySD_ED_Last(startDate, endDate,
				orderByComparator);
	 ...
	 */
	@Override
	public ESFCard fetchByfindBySD_ED_Last(Date startDate, Date endDate,
		OrderByComparator orderByComparator) throws SystemException {
		int count = countByfindBySD_ED(startDate, endDate);

		if (count == 0) {
			return null;
		}

		List<ESFCard> list = findByfindBySD_ED(startDate, endDate, count - 1,
	 ...
	 */
	@Override
	public ESFCard[] findByfindBySD_ED_PrevAndNext(long esfCardId,
		...
		try {
			...
			array[0] = getByfindBySD_ED_PrevAndNext(session, esfCard,
					startDate, endDate, orderByComparator, true);

			array[1] = esfCard;

			array[2] = getByfindBySD_ED_PrevAndNext(session, esfCard,
					startDate, endDate, orderByComparator, false);
	...
	protected ESFCard getByfindBySD_ED_PrevAndNext(Session session,
		ESFCard esfCard, Date startDate, Date endDate,
	 ...
	 */
	@Override
	public void removeByfindBySD_ED(Date startDate, Date endDate)
		throws SystemException {
		for (ESFCard esfCard : findByfindBySD_ED(startDate, endDate,
	 ...
	 */
	@Override
	public int countByfindBySD_ED(Date startDate, Date endDate)
		throws SystemException {
		FinderPath finderPath = FINDER_PATH_COUNT_BY_FINDBYSD_ED;

		Object[] finderArgs = new Object[] { startDate, endDate };
	...
	private static final String _FINDER_COLUMN_FINDBYSD_ED_STARTDATE_1 = "esfCard.startDate IS NULL AND ";
	private static final String _FINDER_COLUMN_FINDBYSD_ED_STARTDATE_2 = "esfCard.startDate = ? AND ";
	private static final String _FINDER_COLUMN_FINDBYSD_ED_ENDDATE_1 = "esfCard.endDate IS NULL";
	private static final String _FINDER_COLUMN_FINDBYSD_ED_ENDDATE_2 = "esfCard.endDate = ?";
	...
	public ESFCard updateImpl(it.ethica.esf.model.ESFCard esfCard)
		...
		if (isNew || !ESFCardModelImpl.COLUMN_BITMASK_ENABLED) {
			...
			if ((esfCardModelImpl.getColumnBitmask() &
					FINDER_PATH_WITHOUT_PAGINATION_FIND_BY_FINDBYSD_ED.getColumnBitmask()) != 0) {
				Object[] args = new Object[] {
						esfCardModelImpl.getOriginalStartDate(),
						esfCardModelImpl.getOriginalEndDate()
					};

				FinderCacheUtil.removeResult(FINDER_PATH_COUNT_BY_FINDBYSD_ED,
					args);
				FinderCacheUtil.removeResult(FINDER_PATH_WITHOUT_PAGINATION_FIND_BY_FINDBYSD_ED,
					args);

				args = new Object[] {
						esfCardModelImpl.getStartDate(),
						esfCardModelImpl.getEndDate()
					};
	...
	protected ESFCard toUnwrappedModel(ESFCard esfCard) {
		...
		esfCardImpl.setStartDate(esfCard.getStartDate());
		esfCardImpl.setEndDate(esfCard.getEndDate());
