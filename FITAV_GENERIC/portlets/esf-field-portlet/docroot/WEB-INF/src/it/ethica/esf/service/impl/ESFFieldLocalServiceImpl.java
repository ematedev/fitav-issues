...
/**
 * The implementation of the ESFField local service.
 *
 * <p>
 * All custom service methods should be put in this class. Whenever methods are added, rerun ServiceBuilder to copy their definitions into the {@link it.ethica.esf.service.ESFFieldLocalService} interface.
 *
 * <p>
...
public class ESFFieldLocalServiceImpl extends ESFFieldLocalServiceBaseImpl {
	...
	public List<ESFField> getESFFields(long groupId) throws SystemException {
	...
	public int getESFFieldsCountByGroupId(long groupId) throws SystemException {
	...
	public List<ESFField> getESFFieldsByESFSportType(long esfSportTypeId)
		...
		List<ESFFieldESFSportType> esfFieldEsfSportTypes = 
				esfFieldESFSportTypePersistence
					.findByesfSportTypeId(esfSportTypeId);

		for (ESFFieldESFSportType esfFieldEsfSportType : esfFieldEsfSportTypes) {

			esfFields.add(esfFieldLocalService
					.getESFField(esfFieldEsfSportType.getEsfFieldId()));
		}
		
		return esfFields;
	}

	public ESFField addESFField(long userId, String name,
			long ownerOrganizationId, long esfAddressIdId,
			long esfSportTypeIds[], ServiceContext serviceContext)
					throws SystemException, PortalException {
		...
		resourceLocalService.addResources(user.getCompanyId(), groupId, userId,
				ESFField.class.getName(), esfFieldId, false, true, true);

		AssetEntry assetEntry = assetEntryLocalService.updateEntry(userId,
				groupId, esfField.getCreateDate(),
	...
	public ESFField updateESFField(long userId, long esfFieldId,
			String name, long ownerOrganizationId, long esfAddressId,
			long esfSportTypeIds[], ServiceContext serviceContext)
		...
		resourceLocalService.updateResources(serviceContext.getCompanyId(),
				serviceContext.getScopeGroupId(), name, esfFieldId,
	...
	public ESFField deleteESFField(long esfFieldId,
		...
		resourceLocalService.deleteResource(serviceContext.getCompanyId(),
				ESFField.class.getName(), ResourceConstants.SCOPE_INDIVIDUAL,
				esfFieldId);
	...
	protected void updateESFFieldESFSportType(long esfFieldId,
			long esfSportTypeIds[], ServiceContext serviceContext)
			throws SystemException {
		...
		for (ESFFieldESFSportType
					esfFieldESFSportType : currentESFFieldESFSportType) {
		...
		for (long esfSportTypeId : esfSportTypeIds) {
			...
			if (Validator.isNotNull(esfSportTypeId) && esfSportTypeId > 0) {
				...
				ESFFieldESFSportTypePK esfFieldEsfSportTypePK = new 
						ESFFieldESFSportTypePK(esfFieldId, esfSportTypeId);

				ESFFieldESFSportType esfFieldESFSportType = null;

				try {

					esfFieldESFSportType = esfFieldESFSportTypePersistence
							.findByPrimaryKey(esfFieldEsfSportTypePK);
				} catch (NoSuchFieldESFSportTypeException e) {

					esfFieldESFSportType = esfFieldESFSportTypePersistence
							.create(esfFieldEsfSportTypePK);
				}

				esfFieldESFSportTypePersistence.update(esfFieldESFSportType, 
						serviceContext);
			}
		}
	}
