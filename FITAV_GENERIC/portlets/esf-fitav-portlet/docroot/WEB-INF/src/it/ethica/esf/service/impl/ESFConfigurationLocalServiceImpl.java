...
public class ESFConfigurationLocalServiceImpl
	...
	public HashMap<String,Integer> findAgeYouthShooter(String youthS) throws Exception{
		
		HashMap<String,Integer> ageByName = new HashMap<String,Integer>();
		List<ESFConfiguration> youthConfiguration = new ArrayList<ESFConfiguration>();
		int age = 0;
		
		DynamicQuery configurationQuery = DynamicQueryFactoryUtil.forClass(
			ESFConfiguration.class, "configurationQuery",
			PortletClassLoaderUtil.getClassLoader());

		configurationQuery.add(PropertyFactoryUtil.forName("configurationQuery.key").like(
			StringPool.PERCENT + youthS + StringPool.PERCENT));
