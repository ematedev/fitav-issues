...
public class ESFShooterPortlet extends MVCPortlet {
	...
	public void render(RenderRequest request, RenderResponse response)
		...
		getCurrentEsfFederalRoles(request, response);
	...
	public void getCurrentEsfFederalRoles(RenderRequest request, RenderResponse response){
		long esfUserId = ParamUtil.getLong(request, "esfUserId", -1);
		int total = 0;
		int start = -1;
		int end = -1;
		List<ESFUserESFFederalRole> associations = new ArrayList<ESFUserESFFederalRole>();
		associations = ESFFederalRoleLocalServiceUtil.getAssociationsByEsfUserActive(esfUserId, Boolean.TRUE);
		total = associations.size();
		request.setAttribute("esfUserId", esfUserId);
		request.setAttribute("currentResults", associations);
		request.setAttribute("currentTotal", total);
	}
	...
	public void getEsfFederalRoles(RenderRequest request, RenderResponse response){
		...
		request.setAttribute("esfUserId", esfUserId);
		request.setAttribute("federalRoles", federalRoles);
		request.setAttribute("federalRolesTotal", total);
	}
