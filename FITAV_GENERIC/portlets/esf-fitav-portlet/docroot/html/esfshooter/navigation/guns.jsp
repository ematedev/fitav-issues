...
<%
	...
	portletURL.setParameter("mvcPath","/html/esfshooter/navigation/guns.jsp");
...
%>
<c:if test="<%= ESFShooterPermission.contains(permissionChecker,esfUserId, ActionKeys.ESFSHOOTER_GUN_ADD) %>">	

	<portlet:renderURL var="addESFRifleURL">
		<portlet:param name="mvcPath" value='<%= "/html/esfshooter/edit_esfRifle.jsp" %>' />
...
<c:if test="<%= ESFShooterPermission.contains(permissionChecker,esfUserId, ActionKeys.ESFSHOOTER_GUN_CANE_ADD) %>">	
	<portlet:renderURL var="addESFCaneURL">
			<portlet:param name="mvcPath" value='<%= "/html/esfshooter/edit_esfCane.jsp" %>' />
			<portlet:param name="esfUserId" value="<%=String.valueOf(esfUserId) %>" />
			<portlet:param name="modifyMode" value="false"/>
	</portlet:renderURL>
...
<c:if test="<%= ESFShooterPermission.contains(permissionChecker,esfUserId, ActionKeys.ESFSHOOTER_GUN_CATRIDGE_ADD) %>">	
	<portlet:renderURL var="addESFCatridgeURL">
		<portlet:param name="mvcPath" value='<%= "/html/esfshooter/edit_esfCatridge.jsp" %>' />
		<portlet:param name="esfUserId" value="<%=String.valueOf(esfUserId) %>" />
		<portlet:param name="modifyMode" value="false"/>
	</portlet:renderURL>
...
<liferay-ui:search-container emptyResultsMessage="no-results" delta="5"  curParam= "sc1">	
		<liferay-ui:search-container-results>
			<%
				List<ESFgunUser> gunUser =	ESFgunUserLocalServiceUtil.getESFGunByLikeS_T(esfUserId, rifle, 
							searchContainer.getStart(), searchContainer.getEnd());
				int	tot = ESFgunUserLocalServiceUtil.getESFGunByLikeS_T(esfUserId, rifle, 
			...
			ESFUser shooter = ESFUserLocalServiceUtil.fetchESFUser(esfGunU.getEsfUserId());
			ESFGunType gunType = ESFGunTypeLocalServiceUtil.fetchESFGunType(esfGunU.getEsfGunnTypeId());
			ESFGunKind kind = ESFGunKindLocalServiceUtil.fetchESFGunKind(esfGunU.getEsfGunId());
			...
			%>
					...
					<c:if test="<%= ESFShooterPermission.contains(permissionChecker,esfUserId, ActionKeys.ESFSHOOTER_GUN_VIEW_ACTION) %>">	
				
						<liferay-ui:search-container-column-jsp
						path='<%="/html/esfshooter/navigation/guns_actions.jsp"%>' align="right" cssClass="gun_actions_button"/>	
...
<h3> <liferay-ui:message key="CANES"/> </h3>		
	<liferay-ui:search-container emptyResultsMessage="no-results" delta="5"  curParam= "sc2">
			...
			ESFUser shooter = ESFUserLocalServiceUtil.fetchESFUser(esfGunU.getEsfUserId());
			ESFCane cane = ESFCaneLocalServiceUtil.fetchESFCane(esfGunU.getEsfGunId());
			ESFGunKind kind = ESFGunKindLocalServiceUtil.fetchESFGunKind(cane.getEsfGunKindId());
			...
			%>
					...
					<c:if test="<%= ESFShooterPermission.contains(permissionChecker,esfUserId, ActionKeys.ESFSHOOTER_GUN_CANE_VIEW_ACTION) %>">	
					
						<liferay-ui:search-container-column-jsp
						path='<%="/html/esfshooter/navigation/esfCane_actions.jsp"%>' align="right" cssClass="gun_actions_button"/>	
...
<liferay-ui:search-container emptyResultsMessage="no-results" delta="5"  curParam= "sc3">
			...
			<liferay-ui:search-container-row
				...
				%>
					...
					<c:if test="<%= ESFShooterPermission.contains(permissionChecker,esfUserId, ActionKeys.ESFSHOOTER_GUN_CATRIDGE_VIEW_ACTION) %>">	
					
						<liferay-ui:search-container-column-jsp
						path='<%="/html/esfshooter/navigation/esfCatridge_actions.jsp"%>' align="right" cssClass="gun_actions_button"/>	
