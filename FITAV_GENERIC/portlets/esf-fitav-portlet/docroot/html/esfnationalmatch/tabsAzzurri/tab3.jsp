...
<%
/*List<ESFNational> shootersTab3 = ESFNationalLocalServiceUtil.findNationalbySportId_MatchId(idSport.get(2), esfMatchId,
		searchContainer.getStart(),searchContainer.getEnd());
int totTab3 = ESFNationalLocalServiceUtil.countNationalbySportId_MatchId(idSport.get(2), esfMatchId);*/

List<ESFNational> shootersTab3 = ESFNationalLocalServiceUtil.findNationalFreeMatchIdSportId(esfMatchId, idSport.get(2), searchContainer.getStart(),searchContainer.getEnd());
int totTab3 = ESFNationalLocalServiceUtil.findNationalFreeMatchIdSportId(esfMatchId, idSport.get(2), 0, Integer.MAX_VALUE).size();

List<ESFNational> shootersOrdTab3 = new ArrayList<ESFNational>(shootersTab3);

if (Validator.isNotNull(orderByCol)) {
	BeanComparator beanComparator = new BeanComparator(orderByCol);
	
	Collections.sort(shootersOrdTab3,beanComparator);
	
	if (sortingOrder.equalsIgnoreCase("desc")) {
	
		Collections.reverse(shootersOrdTab3);
	}
}

pageContext.setAttribute("results", shootersOrdTab3);
pageContext.setAttribute("total", totTab3);
		...
		%>		
				...
				<liferay-ui:search-container-column-text name = "shooter-qualification" >
					<aui:form action = "<%=addESFNationalDelegationURL%>">
						...
						<aui:input name="sportTypeId" value="<%=String.valueOf(idSport.get(2)) %>" type="hidden"></aui:input>
